...
import { AppInsightsCore, IConfiguration, ITelemetryItem, ITelemetryPlugin, IChannelControls, _InternalMessageId, CoreUtils, getPerformance, getGlobalInst, getGlobal } from "@microsoft/applicationinsights-core-js";
 ...
 */
class SimpleSyncPromise<T> {
    public then: (onResolved: (value?: T | SimpleSyncPromise<T> | void) => void) => SimpleSyncPromise<T>;

    constructor(executor: (resolve: (value?: T | SimpleSyncPromise<T> | void) => void, reject?: (reason?: any) => void) => void) {
        let _self = this;
        let _state = "pending";
        let _settledValue:T = null;
        let _queue = [];

        _self.then = (onResolved: (value?: T | SimpleSyncPromise<T>) => void) => {
            return new SimpleSyncPromise((resolve: (value?: T | SimpleSyncPromise<T> | void) => void) => {
                // Queue the new promise returned to be resolved or rejected
                // when this promise settles.
                _enqueue(onResolved, resolve);
            });
        };

        function _enqueue(onResolved: (value?: T | SimpleSyncPromise<T>) => void, resolve: (value?: T | SimpleSyncPromise<T> | void) => void) {
            _queue.push(() => {
                let value:T|SimpleSyncPromise<T>|void;
...
function hookFetch<T>(executor: (resolve: (value?: T | PromiseLike<T>) => void, reject: (reason?: any) => void) => void) {
    ...
    global.fetch = function(input: RequestInfo, init?: RequestInit) {
        return new SimpleSyncPromise(executor);
    }
}
        ...
        this.testCaseAsync({
            ...
            .concat(PollingAssert.createPollingAssert(() => {
                ...
                if (trackStub.called) {
                    ...
                    Assert.equal(undefined, props.perf, "Should contain properties perf object");
                    return true;
                }
        ...
        this.testCaseAsync({
            ...
            .concat(PollingAssert.createPollingAssert(() => {
                ...
                if (trackStub.called) {
                    ...
                    Assert.notEqual(undefined, props, "Should contain properties");
                    if (props) {
                        Assert.notEqual(undefined, props.perf, "Perf detail exists")
                        let perf = props.perf || {};
                        if (perf.missing) {
                            Assert.equal(true, !!perf.missing, "Performance was executed but browser did not populate the window.performance entries - " + JSON.stringify(data));
                        } else {
